...
func TestVerifyBundlesAtomicity(t *testing.T) {
	tests := []struct {
		...
		// forcedTxs is a map of tx hashes from mempool
		forcedTxsHashes map[common.Hash]forcedTxData
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := checkBundlesAtomicity(test.includedBundles, test.includedTxDataByHash, test.privateTxData, test.mempoolTxHashes, test.forcedTxsHashes)
...
func TestExtractBundleDataFromUsedBundles(t *testing.T) {
	...
	bundle := builderTypes.SimulatedBundle{
		OriginalBundle: builderTypes.MevBundle{
	...
	extractBundleTxDataFromBundles([]builderTypes.SimulatedBundle{bundle}, result)
...
func BenchmarkVerifyBundlesAtomicity(b *testing.B) {
	...
	var (
		env              = &environment{}
		committedBundles []builderTypes.SimulatedBundle
		allBundles       []builderTypes.SimulatedBundle
		mempoolTxHashes  = make(map[common.Hash]struct{})
		forcedTxs        types.Transactions
	)
	...
	for i := 0; i < 1000; i++ {
		...
		committedBundles = append(committedBundles, builderTypes.SimulatedBundle{
			OriginalBundle: builderTypes.MevBundle{
		...
		allBundles = append(allBundles, builderTypes.SimulatedBundle{
			OriginalBundle: builderTypes.MevBundle{
				Txs:               types.Transactions{tx},
				RevertingTxHashes: []common.Hash{},
				Hash:              bundleHash,
	...
	// generate forced txs
	for i := 0; i < 1000; i++ {
		data := crypto.Keccak256([]byte(fmt.Sprintf("forced-tx-%x", i)))
		tx := signers.signTx(1, 21000, big.NewInt(0), big.NewInt(1), signers.addresses[2], big.NewInt(0), data)
		hash := tx.Hash()
		env.txs = append(env.txs, tx)
		env.receipts = append(env.receipts, &types.Receipt{TxHash: hash, Status: types.ReceiptStatusSuccessful})
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		err := VerifyBundlesAtomicity(env, committedBundles, allBundles, mempoolTxHashes, forcedTxs)
		if err != nil {
			b.Fatal(err)
		}
	}
